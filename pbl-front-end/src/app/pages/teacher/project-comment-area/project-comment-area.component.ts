...
export class ProjectCommentAreaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params=>{
      ...
      this.project.getComments(this.project_id).subscribe(res=>{
        this.comments=res;
        this.initData();
        this.teacher.getTeacherInfo().subscribe(stu=>{
          this.teacherInfo=stu;
        });
      });
    });
  }
  ...
  handleSubmit(comment:any): void {
    ...
    this.project.addComment({
      author_id:this.teacherInfo.id,
      author_name:this.teacherInfo.name,
      date:tmp,
      reply:comment.id,
      content:content,
